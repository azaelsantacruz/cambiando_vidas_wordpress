...
;(function ( $, window, document, undefined ) {
	...
	PT_Builder.prototype = {
		_getContent: function(){
		    if ( tinyMCE.get('content') && $("#wp-content-wrap").hasClass("tmce-active") ){
		...
		init: function(){
			THIS.$element.append('<div class="pt-builder-wrapper"> '+
									'<div class="pt-main-actions clearfix">'+
										'<a href="javascript:;" class="button pt-template">Templates</a>'+
										'<a href="javascript:;" class="button pt-css">CSS</span></a>'+
										'<a href="javascript:;" class="button pt-preview">Preview</a>'+
										'<a href="javascript:;" class="button button-primary pt-save"></a>'+										
									'</div>'+
									'<div class="pt-builder-content"></div>'+
										'<a href="javascript:;" class="button pt-add" data-contain_shortcode_element="PT_Section">Add Section</a>'+
			...
			$(document).on( 'hover', '.pt-element-item', function(e){
				...
				$('.pt-element-description').html( '<label>'+$this.find('.pt-element-text p').html()+'</label>' + decoded );
			});
		...
		destroy: function(){
			...
			$('#postdivrich').show();
			switchEditors.switchto($('#wp-content-wrap')[0]);
			THIS._setContent();
			THIS.$element.remove();
