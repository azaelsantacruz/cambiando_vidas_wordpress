...
class PT_Content_Slider extends PT_Shortcode{
	...
	public function shortcode_frontend( $atts, $content ){
		...
		if( !empty( $images ) || !empty( $content ) ){
			...
			if( !empty( $images ) ){
				...
				for( $i=0; $i<sizeof( $image_ids ); $i++ ){
					...
					if( !empty( $image_data ) ){
						...
						$indicators .= '<li data-target="#'.$random_string.'" data-slide-to="'.$i.'" class="'.( $i == 0 ? 'active' : '' ).'"></li>';
			...
			$html = '<div id="'.$random_string.'" class="carousel slide '.$extra_class.'" data-ride="carousel" data-interval="'.$delay.'">';
			if( $navigation == 'yes' ){
			...
			if( $arrows == 'yes' ){
				$html .= '
                    <a class="left carousel-control" href="#'.$random_string.'" role="button" data-slide="prev">
                        <span class="glyphicon glyphicon-chevron-left"></span>
                    </a>
                    <a class="right carousel-control" href="#'.$random_string.'" role="button" data-slide="next">
