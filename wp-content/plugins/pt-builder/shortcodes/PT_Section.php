...
class PT_Section extends PT_Shortcode{	
	...
	public function shortcode_admin( $atts, $content ){
		...
		return '<div class="pt-section pt-element-root" data-shortcode_id="'.$atts['shortcode_id'].'" data-shortcode_element="PT_Section" data-contain_shortcode_element="PT_Row">
					<div class="pt-actions clearfix">						
						<a href="javascript:;" class="pt-section-drag" title="'.__( 'Drag Section', 'pt-builder' ).'"><span class="fa fa-arrows"></span></a>
					...
					<div class="pt-collapsible">
						<div class="pt-section-content '.$atts['shortcode_id'].'">
							'.do_shortcode( $content ).'
						</div>
						<a href="javascript:;" class="button pt-add">Add Row</a>
					</div>
				</div>';	
	}
	...
	public function shortcode_frontend( $atts, $content ){
		...
		return '<section style="'.( $width == 'boxed' ? '' : $style ).'" class="'.$extra_class.'">
					<div class="container'.( $width == 'boxed' ? '' : '-fluid' ).'" style="'.( $width == 'boxed' ? $style : '' ).' max-width: 100%;">
						'.do_shortcode( $content ).'
					</div>
				</section>';
	}
