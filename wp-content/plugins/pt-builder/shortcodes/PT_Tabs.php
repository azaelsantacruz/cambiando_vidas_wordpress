	...
	public function shortcode_frontend( $atts, $content ){
		...
		if( !empty( $titles ) ){
			...
			for( $i=0; $i<sizeof($titles); $i++ ){
				...
				$navigation .= '<li '.( $i == 0 ? 'class="active"' : '' ).'><a href="#'.$random_string.'_'.$i.'" role="tab" data-toggle="tab">'.$title.'</a></li>';
				$tabs .= '<div class="tab-pane '.( $i == 0 ? 'active' : '' ).'" id="'.$random_string.'_'.$i.'">'.apply_filters( 'the_content', $tabs_content ).'</div>';
			}
		...
		return '<div class="'.$extra_class.'">'.$html.'</div>';
	}
