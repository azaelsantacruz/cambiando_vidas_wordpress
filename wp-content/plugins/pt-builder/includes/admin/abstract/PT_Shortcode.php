...
class PT_Shortcode{
	...
	public function shortcode_admin( $atts, $content ){
		...
		return '<div class="pt-element pt-element-root" data-shortcode_id="'.$atts['shortcode_id'].'" data-shortcode_element="'.get_class( $this ).'">
	...
	public function shortcode_frontend( $atts, $content ){
		...
		return '
			'.$style.'
			<a class="btn btn-default '.$size.' '.$random_string.'" href="'.$link.'" target="'.$target.'">
				'.( !empty( $icon ) ? '<span class="fa '.$icon.'"></span> ' : '' ).'
				'.$text.'
			</a>
		';
	}
