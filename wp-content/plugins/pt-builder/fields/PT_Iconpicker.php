...
class PT_Iconpicker extends PT_Field{
	...
	public function generate_html(){
		...
		foreach( $icons as $icon_key => $icon_name ){
			if( $icon_key != 'none' ){
				$icons_html .= '<li data-icon_name="'.$icon_key.'" class="'.( $icon_key == $value ? 'active' : '' ).'"><span class="fa '.$icon_key.'"></span></li>';
			}
		}
		...
		$this->field_html = '
			<div class="pt-option-container">
				...
				<a href="javascript:;" class="button pt-icons-select">'.( !empty( $value ) ? '<span class="fa '.$value.'"></span> ' : '' ).__( 'Select Icon', 'pt-builder' ).'</a>
				'.(
					!empty( $value ) ? '<a href="javascript:;" class="button pt-icons-clear">'.__( 'Clear', 'pt-builder' ).'</a>' : ''
				).'
				<input type="hidden" id="'.$id.'" class="pt-option pt-iconpicker" value="'.$value.'" />
