...
var SM = {
	...
	init : function() {
		...
		jQuery(document).ready( function () {
			...
			jQuery( document ).on( 'click', '.repeat-field', function(e) {				e.preventDefault();				if(jQuery(this).parent().find('.field-item:eq(0)').attr('data-class') != 'SM_Image_Field'){
	...
	repeatField : function( field ) {
	    ...
	    newT.find( '.sm_upload_status' ).html('');
		...
		field.children( '.field-item' ).not( templateField ).each( function() {
			...
			var search  = field.hasClass( 'SM_Group_Field' ) ? /sm-group-(\d|x)*/g : /sm-field-(\d|x)*/g;
			var replace = field.hasClass( 'SM_Group_Field' ) ? 'sm-group-' + index : 'sm-field-' + index;
	...
	isMaxFields: function( field, modifier ) {
		...
		field.find( '> .field-item > .sm-delete-field, > .field-item > .group > .sm-delete-field' ).show();
	...
	/**
	 * Fire init callbacks.
	 * Called when SM has been set up.
	 */
	doneInit: function() {
	...
	addCallbackForClonedField: function( fieldName, callback ) {
		...
		if ( jQuery.isArray( fieldName ) )
			for ( var i = 0; i < fieldName.length; i++ )
				SM.addCallbackForClonedField( fieldName[i], callback );
	...
	addCallbackForDeletedField: function( fieldName, callback ) {
		...
		if ( jQuery.isArray( fieldName ) )
			for ( var i = 0; i < fieldName.length; i++ )
				SM._deletedFieldCallbacks( fieldName[i], callback );
