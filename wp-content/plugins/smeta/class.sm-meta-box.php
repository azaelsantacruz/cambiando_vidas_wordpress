...
class SM_Meta_Box {
	...
	function __construct( $meta_box ) {
		...
		add_action( 'sm_init_fields', array( &$this, 'init_fields' ) );
		global $pagenow;
		if ( $upload && in_array( $pagenow, array( 'page.php', 'page-new.php', 'post.php', 'post-new.php' ) ) )
			add_action( 'admin_head', array( &$this, 'add_post_enctype' ) );
		add_action( 'admin_menu', array( &$this, 'add' ) );
		add_action( 'save_post', array( &$this, 'save_for_post' ) );
		add_action( 'sm_save_fields', array( &$this, 'save' ) );
		add_action( 'admin_enqueue_scripts', array( &$this, 'enqueue_scripts' ) );
		add_action( 'admin_enqueue_scripts', array( &$this, 'enqueue_styles' ) );
		add_filter( 'sm_show_on', array( &$this, 'add_for_id' ), 10, 2 );
		add_filter( 'sm_show_on', array( &$this, 'add_for_page_template' ), 10, 2 );
	}
	public function init_fields( $post_id = 0 ) {
		foreach ( $this->_meta_box['fields'] as $key => $field ) {
			...
			$class = _sm_field_class_for_type( $field['type'] );
	...
	function enqueue_scripts() {
		wp_enqueue_script( 'sm-scripts', trailingslashit( SM_URL ) . 'js/sm.js', array( 'jquery' ) );
		foreach ( $this->fields as $field )
			$field->enqueue_scripts();
	}
	function enqueue_styles() {
		wp_enqueue_style( 'sm-styles', trailingslashit( SM_URL ) . 'style.css' );
		foreach ( $this->fields as $field )
	...
	function add() {
		...
		foreach ( (array) $this->_meta_box['pages'] as $page )
			if ( apply_filters( 'sm_show_on', true, $this->_meta_box ) )
				add_meta_box( $this->_meta_box['id'], $this->_meta_box['title'], array(&$this, 'show'), $page, $this->_meta_box['context'], $this->_meta_box['priority'] ) ;
	}
	/**
	 * Show On Filters
	 * Use the 'sm_show_on' filter to further refine the conditions under which a metabox is displayed.
	 ...
	 *
	 * @param  SM_Field[]  $fields
	 */
	static function layout_fields( array $fields ) { ?>
		<table class="form-table sm_metabox">
			...
			foreach ( $fields as $field ) :
				...
				<?php endif; ?>
				<input type="hidden" name="_sm_present_<?php esc_attr_e( $field->id ); ?>" value="1" />
			<?php endforeach; ?>
		</table>
	<?php }
	function strip_repeatable( $values ) {
		foreach ( $values as $key => $value ) {
			if ( false !== strpos( $key, 'sm-group-x' ) || false !==  strpos( $key, 'sm-field-x' ) )
				unset( $values[$key] ); 
	...
	function save( $post_id = 0 )  {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( ! isset( $_POST['_sm_present_' . $field['id'] ] ) )
				continue;
			...
			if ( ! $class = _sm_field_class_for_type( $field['type'] ) ) {
				do_action('sm_save_' . $field['type'], $field, $value);
			}
			if ( !empty(  $this->_meta_box['repeatable'] ) )
				$field['repeatable'] = true;
