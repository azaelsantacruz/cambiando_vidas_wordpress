...
class PayPal{
	...
	public function __construct( $data ){
		$this->mode = gorising_get_option( 'paypal_mode' );
		$this->url = "https://api-3t".$this->mode.".paypal.com/nvp";
		$this->version = urlencode('109.0');
		if( !empty( $data['username'] ) && !empty( $data['password'] ) && !empty( $data['signature'] ) && !empty( $data['returnUrl'] ) && !empty( $data['cancelUrl'] ) ){
			...
			$this->exception( 'API username, API password, APi signature and Cancel URL can not be blank.' );
		}
